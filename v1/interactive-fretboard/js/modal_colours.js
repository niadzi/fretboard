...
/**
 * [GuitarString description]
 * @param {[ string | int ]}    rootNote     [description]
 * @param {[ int ]}             stringNumber [description]
 */
var GuitarString = function(rootNote, stringNumber) {
    ...
    this.draw = function() {
        ...
        for (var i = 1; i < NUM_FRETS+1; i++) {
            ...
            fretHtml.style.width = (fretWidths[i]*.091)+"%";  
    ...
    this.displayInterval = function(interval) {
        ...
        for (var i = 1; i < NUM_FRETS+1; i++) {
            var fretHtml        = document.getElementById('s-' + stringNumber + '-f-' + i);
            var dataInterval    = (i + this.rootOffset ) % 12;
            fretHtml.setAttribute("data-tone", dataInterval);
            fretHtml.setAttribute("data-interval", dataInterval);
            var fretHighlight = document.getElementById('highlight-s-' + stringNumber + '-f-' + i);
            var hue = getHue( dataInterval, 12); //fretHighlight.style.background = "transparent";   
            fretHighlight.style.background = "hsla("+hue+", 100%, 70%,90)";
            var fretNoteHtml = document.getElementById('fretnote-s-' + stringNumber + '-f-' + i);
                ...
                if(fretHtml !== null) {
                 ...
                 //  console.info("!!new interval" + newInterval);
                    if(selectedIntervals[newInterval] == "on"){
                        fretHighlight.style.background = "hsla("+hue+", 100%, 70%,90)";
                        fretHighlight.style.borderColor = "1px solid hsla("+COLOURS[intervalColour]+", 70%, 95%,90)"; 
