...
var GuitarString = function (rootNote, stringNumber) {
  ...
  this.refresh = function () {
    var fretNote = Notes[(i + this.rootOffset) % 12];
    var stringHtml = document.getElementById(
      "string-" + stringNumber + "fretsList",
    );
    for (var i = 1; i < NUM_FRETS + 1; i++) {
      var fretHtml = document.getElementById("s-" + stringNumber + "-f-" + i);
      fretHtml.setAttribute("data-tone", (i + this.rootOffset) % 12);
      fretHtml.setAttribute("data-interval", (i + this.rootOffset) % 12);
      var fretHighlight = document.getElementById(
        "highlight-s-" + stringNumber + "-f-" + i,
      );
      ...
      var fretNoteHtml = document.getElementById(
        "fretnote-s-" + stringNumber + "-f-" + i,
      );
  ...
  this.update_ = function (newScaleRoot) {
    var stringHtml = document.getElementById("string-" + stringNumber);
    var fretsList = document.getElementById(
      "string-" + stringNumber + "fretsList",
    );
    ...
    for (var i = 1; i < NUM_FRETS + 1; i++) {
      ...
      var fretHtml = document.getElementById("s-" + stringNumber + "-f-" + i);
      fretHtml.className = "fret";
      fretHtml.setAttribute(
        "data-interval",
        (i + this.rootOffset + newScaleRoot) % 12,
      );
      ...
      if ((i + this.rootOffset + newScaleRoot) % 12 == 0) {
        var hue = getHue(0, 12);
        fretHighlight.style.background =
  ...
  this.displayInterval = function (interval) {
    console.log(
      "Displaying intervals on " + "string-" + stringNumber + "-" + interval,
    );
    ...
    for (var stringNumb = 1; stringNumb < 7; stringNumb++) {
      ...
      for (var i = 0; i < NUM_FRETS; i++) {
        ...
        if (fretHtml !== null) {
          ...
          var fretHighlight = document.getElementById(
            "highlight-s-" + stringNumb + "-f-" + i,
          );
          var fretTone = fretHtml.dataset.tone;
          var newInterval =
            Math.abs(parseInt(parseInt(fretTone) - parseInt(SCALE_ROOT) + 12)) %
            12;
          ...
          if (selectedIntervals[newInterval] == "on") {
            fretHighlight.style.background = "hsla(" + hue + ", 100%, 70%,90)";
            fretHighlight.style.borderColor =
              "1px solid hsla(" + hue + ", 70%, 95%,90)";
            fretHighlight.style.opacity = "1";
            fretHighlight.style.background =
              "hsla(" + modeColours[newInterval] + ", 70%, 95%,90)";
          } else {
            fretHighlight.style.background = "hsla(" + hue + ", 70%, 90%,0.1)";
            fretHighlight.style.border =
              "0px solid hsla(" + hue + ", 100%, 70%,90)";
            fretHighlight.style.opacity = "0.6";
            //     fretHighlight.style.background = "hsla("+INTERVAL_COLOURS[intervalColour]+", 70%, 95%,90)";
          }
        }
        if (i == 0) {
          fretHighlight.style.background = "transparent";
...
var colourIntervalButtons = function () {
  for (var i = 0; i < 13; i++) {
    ...
    var button = $("ul#intervals-button-list li:nth-child(" + i + ")")
      .find("button")
      .css("background", "#777777");
    var button = $("ul#intervals-button-list li:nth-child(" + i + ")")
      .find("button.selected-interval")
      .css("background", "hsla(" + hue + ", 100%, 70%,90)");
  }
};
...
var toggleInterval = function (interval) {
  ...
  } else if (button.dataset.status == "on") {
    button.className = "";
  ...
  colourIntervalButtons();
  console.log(
    "toggleInterval(" + Intervals[interval] + ") / selectedIntervals: ",
  );
...
var turnIntervalOn = function (interval) {
  ...
  colourIntervalButtons();
  console.log(
    "turnIntervalOn(" + Intervals[interval] + ") / selectedIntervals: ",
  );
